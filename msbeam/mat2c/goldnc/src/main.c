...
void MumfordShah(double alpha,double beta,double epsilon) {
    ...
    for (k = 1; k<=_IterAlt; ++k) {
        ...
        for (i = 0; i<imageHight; ++i) {
            for (j = 0; j<imageWidth; ++j) {
                MSE += sqr(init_f[i][j]-f[i][j]);
            }
        }
        printf("MSE = %f MSE(mean) = %f\n",MSE,MSE/(imageHight*imageWidth));
    }
    printf("Leaving MumfordShah_reg...\n");
}

void trans(double f[imageHight][imageWidth]) {
    double fmin = 255.0,fmax = -255.0;
    int i,j;
    for (i = 0; i<imageHight; ++i)
        for (j = 0; j<imageWidth; ++j) {
            if (fmin>f[i][j]) fmin = f[i][j];
            if (fmax<f[i][j]) fmax = f[i][j];
        }
    for (i = 0; i<imageHight; ++i)
        for (j = 0; j<imageWidth; ++j)
            f[i][j] = (f[i][j]-fmin)/(fmax-fmin)*255.0;
}
...
int main() {
    ...
    for (i = 0; i<imageHight; ++i)
        for (j = 0; j<imageWidth; ++j) {
            ...
            init_MSE += sqr(init_f[i][j]);
        }
    fclose(fin);
    printf("MSE = %f MSE(mean) = %f\n",init_MSE,init_MSE/(imageHight*imageWidth));
